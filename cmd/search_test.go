...
func TestSearch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			args:   []string{"search", "does-not-exist"},
			format: outputTable,
			output: "Unique ID\tDescription\tTags\tSite\t\n---------\t-----------\t----\t----\t\n",
		},
		{
			args:   []string{"search", "does-not-exist", "--output=json"},
			format: outputJSON,
			output: "[]",
		},
		{
			args:   []string{"search", "id"},
			format: outputTable,
			setup: func() error {
		...
		{
			args:   []string{"search", "id", "--output=json"},
			format: outputJSON,
			output: `[
				{
					"unique_id": "id1",
		...
		{
			args:   []string{"search", "desc", "--output=json"},
			format: outputJSON,
			output: `[
				{
					"unique_id": "id1",
		...
		{
			args:   []string{"search", "tag2", "--output=json"},
			format: outputJSON,
			output: `[
				{
					"unique_id": "id1",
		...
		{
			args:   []string{"search", "supersite", "--output=json"},
			format: outputJSON,
			output: `[
				{
					"unique_id": "id1",
		...
		{
			args:   []string{"search", "myId", "--output=json"},
			format: outputJSON,
			output: "[]",
		},
		{
